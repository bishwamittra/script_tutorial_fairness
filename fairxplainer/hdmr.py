...
def analyze(queue, problem: Dict, X: np.ndarray, Y: np.ndarray,
            maxorder: int = 2, maxiter: int = 1000,
            m: int = 10, K: int = 20, R: int = None, alpha: float = 0.95,
            lambdax: float = 0.01,
    ...
    # queue.put([Si.to_df(), Y_em])
    # assert len(Em['RMSE']) == 1, Em['RMSE']
    queue.put([Si.to_df(), Em['RMSE'].mean()])
...
def _compute(X, Y, settings, init_vars):
    ...
    for k in range(K):
        ...
        # 2nd order component functions: individual (backfitting implemented)
        if (maxorder > 1):
        ...
        # 3rd order component functions: individual (backfitting implemented)
        if (maxorder == 3):
            Y_em[:, j3], Em['C3'][:, :, k] = _third_order(Em['B3'][idx[:, k], :, :], Y_res,
                                                          Em['C3'][:, :, k], R, Em['n3'], m3, maxiter, lambdax)
        ...
        # RMSE of emulator
        Em['RMSE'][k] = np.sqrt(
            np.sum(np.square(Y_id - Em['Y_e'][:, k].reshape(-1, 1))) / R)
...
def _init(X, Y, settings):
    ...
    norms = np.tile((X.max(0)) - X.min(0), (N, 1))
    nonzero = norms > 0
    X_n = (X - np.tile(X.min(0), (N, 1)))
    # handle divide by zero
    X_n[nonzero] /= norms[nonzero]
...
def _second_order(B2, Y_res, C2, R, n2, m2, maxiter, lambdax):
    ...
    for j in range(n2):
        # Regularized least squares inversion ( minimize || C1 ||_2 )
        B2_j[:, :] = B2[:, :, j]
        B22 = np.matmul(np.transpose(B2_j), B2_j)

        # if it is ill-conditioned matrix, the default value is zero
        if np.all(np.linalg.svd(B22)[1]):  # sigma, diagonal matrix, from svd
            try:
                T2[:, :, j] = np.linalg.solve(np.add(B22, np.multiply(
                    lambdax, np.identity(m2))), np.transpose(B2_j))
            except Exception as e:
                pass
                # print("Exception:", j, e)
        C2[:, j] = np.matmul(T2[:, :, j], Y_res).reshape(m2,)
        Y_ij[:, j] = np.matmul(B2_j, C2[:, j])

    if(True):
        # Backfitting Method
        it = 0                        # Initialize iteration counter
        var1b_old = np.sum(np.square(C2), axis=0)
        varmax = 1
        while (varmax > 1e-1) and (it < maxiter):
            for j in range(n2):
                Y_r = Y_res
                for z in range(n2):
                    if j != z:
                        Y_r = np.subtract(Y_r, np.matmul(
                            B2[:, :, z], C2[:, z]).reshape(R, 1))

                C2[:, j] = np.matmul(T2[:, :, j], Y_r).reshape(m2,)

            var1b_new = np.sum(np.square(C2), axis=0)
            varmax = np.max(np.absolute(np.subtract(var1b_new, var1b_old)))
            var1b_old = var1b_new
            it += 1
        #     print(it, varmax)
...
def _third_order(B3, Y_res, C3, R, n3, m3, maxiter, lambdax):
    ...
    for j in range(n3):
        # Regularized least squares inversion ( minimize || C1 ||_2 )
        B3_j[:, :] = B3[:, :, j]
        B33 = np.matmul(np.transpose(B3_j), B3_j)

        # if it is ill-conditioned matrix, the default value is zero
        if np.all(np.linalg.svd(B33)[1]):  # sigma, diagonal matrix, from svd
            try:
                T3[:, :, j] = np.linalg.solve(np.add(B33, np.multiply(
                    lambdax, np.identity(m3))), np.transpose(B3_j))
            except Exception as e:
                pass
                # print("Exception:", j, e)

        C3[:, j] = np.matmul(T3[:, :, j], Y_res).reshape(m3,)
        Y_ijk[:, j] = np.matmul(B3_j, C3[:, j])
    
    if(True):
        # Backfitting Method
        it = 0                        # Initialize iteration counter
        var1b_old = np.sum(np.square(C3), axis=0)
        varmax = 1
        while (varmax > 1e-1) and (it < maxiter):
            for j in range(n3):
                Y_r = Y_res
                for z in range(n3):
                    if j != z:
                        Y_r = np.subtract(Y_r, np.matmul(
                            B3[:, :, z], C3[:, z]).reshape(R, 1))

                C3[:, j] = np.matmul(T3[:, :, j], Y_r).reshape(m3,)

            var1b_new = np.sum(np.square(C3), axis=0)
            varmax = np.max(np.absolute(np.subtract(var1b_new, var1b_old)))
            var1b_old = var1b_new
            it += 1
        #     print(it, varmax)
    ...
    for j in range(n3):
        ...
        # Subtract each first order term from residuals
        Y_res = np.subtract(Y_res, Y_ijk[:, j].reshape(R, 1))
...
def _finalize(problem, SA, Em, d, alpha, maxorder, RT, Y_em, bootstrap_idx, X, Y):
    ...
    # print(np.mean(SA['V_Y'], axis=0))
    # print()
    # print(np.mean(SA['Sa'], axis=1))
...
def emulate(self, X, Y=None):
    ...
    # print("RSME on emulated data:", np.square((Y - emulated)).sum())
    # print(emulated)
